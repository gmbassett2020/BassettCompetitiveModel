   ...
   if (iRound == roundCount - 2):
      ...
      for rankKey in sorted(powerRankings.keys()):
         ...
         for iRank in range(len(powerRankings[rankKey])):
            ...
            teamObject = team.Team.teamObjectById[teamId]
            print(str(iRank+1)+" "+teamObject.teamName)
      ...
      for rankKey in sorted(earnedRankings.keys()):
         print("::: earned rankings for "+rankKey)
   ...
   if (thisGameSeason.seasonInfo.get(thisGameSeason.roundNames[iRound]) != None):
      ...
      for game in thisGameSeason.seasonInfo[thisGameSeason.roundNames[iRound]]:
         ...
         if (iRound <= roundCount-2):
            ...
            if (iRound == roundCount-2):
               if (game.score1 >= game.score2):
                  print("results: prob="+str(resultProb)+" "+game.team1Object.teamName+"--"+game.team2Object.teamName+" actual="+str(game.score1)+"--"+str(game.score2)+" predicted="+str(scoreAndProb["score1"])+"--"+str(scoreAndProb["score2"]))
               else:
                  print("results: prob="+str(resultProb)+" "+game.team2Object.teamName+"--"+game.team1Object.teamName+" actual="+str(game.score2)+"--"+str(game.score1)+" predicted="+str(scoreAndProb["score2"])+"--"+str(scoreAndProb["score1"]))
         if (iRound == roundCount-1 and iRound <= thisGameSeason.roundCount-1):
            # Predictions
            if (scoreAndProb["score1"] >= scoreAndProb["score2"]):
               print("predictions: prob="+str(scoreAndProb["probability"])+" "+game.team1Object.teamName+"--"+game.team2Object.teamName+" predicted="+str(scoreAndProb["score1"])+"--"+str(scoreAndProb["score2"])+" spread="+str(scoreAndProb["score1"]-scoreAndProb["score2"]))
            else:
               print("predictions: prob="+str(1.-scoreAndProb["probability"])+" "+game.team2Object.teamName+"--"+game.team1Object.teamName+"  predicted="+str(scoreAndProb["score2"])+"--"+str(scoreAndProb["score1"])+" spread="+str(scoreAndProb["score2"]-scoreAndProb["score1"]))
